...
void Sound::playSound(uint32 id, byte *sound, uint32 size) {
}
...
void Sound::loadSection(uint8 pSection) {
	...
	_system->loadSFXSection(pSection);
}
...
bool Sound::startSpeech(uint16 textNum) {
	...
	int size = _speechFS->getFileSize(speechFileNum + 50000);

	if (mem && size)
		_system->playSpeech(mem, size);
