...
#ifndef OTHSYS_OPENGL_H
#define	OTHSYS_OPENGL_H
...
struct Animation {
	GLuint frame_handle[MAX_FRAMES];
	int width;
	int height;
	int num_frames;
};
...
class OtherSystem_OpenGL : public OtherSystem {
public:
	OtherSystem_OpenGL(int width, int height);
	virtual ~OtherSystem_OpenGL();
	...
	//virtual void setMouseCursor(const byte *buf, int w, int h, int hotspotX, int hotspotY);
	...
	virtual void alContextHack(void);
...
protected:
	...
	ALuint _alSource[AUDIO_NUM_BUFFERS];
	ALuint _alBuffer[AUDIO_NUM_BUFFERS];
	ALCcontext *_alContext;
	ALCdevice *_alDevice;

	ALuint _sfxBuffer[MAX_NUM_SFX];
	ALuint _uiSFXBuffer[NUM_UI_SOUNDS];
...
//	bool _mouseVisible;
	...
	uint16 *_screenTexBuf;	//16-bit screen texture buffer
//	uint32 *_mouseTexBuf;	//32-bit mouse texture buffer

	//texture handles
	GLuint _gameScreenTexture;
//	GLuint _mouseTexture;

	uint16 _pal16[256];		//16-bit version (for main game)
